...
// Class Name   : Cm001_01LogonController
//
...
/**
 * <pre>
 * このクラスはcm001_01のログオンボタンコントローラークラスです。
 * </pre>
 *
 * @author 林　晃平
 * @version 2.0.0
...
@RequestMapping(value = "/Cm001_01Logon.form")
public class Cm001_01LogonController extends Cm001_01BaseController {

    /** Log */
    private static Log log = LogFactory.getLog(Cm001_01LogonController.class);
    ...
    public String executeAction(Model model, Cm001_01Form form, Errors errors, HttpServletRequest request,
        ...
        log.debug("@@@@Cm001_01LogonController");
        // 画面入力項目をCm001_01DTOに設定する。
        Cm001_01DTO inDTO = new Cm001_01DTO();
        // ・"logon"　→　発生元イベント名
        inDTO.setEventName(Cm001Constants.EVENT_NAME_LOGON);
        // ・画面.利用者ID → 利用者ID
        ...
        // セッションに共通情報（PNCommonInfo）を保存する。
        // ・保存キー　・・・　pnCommonInfo
        // 共通情報（PNCommonInfo）を初期化後、DTOから情報を設定する。
        ...
        // DTO.遷移先画面名を判定し遷移先を決定する。
        String nextScr = null;
        // ・「ut001_01：お知らせ（お知らせ照会結果画面）」の場合
        if (outDTO.getNextScreenName().equals(Cm001Constants.NEXT_SCREENNAME_UT001_01)) {
            // 　Ut001_01Init
            nextScr = Cm001Constants.EVENT_ID_CM001_01LOGON1;
        }
        else {
            // ・「cm001_01：パスワード変更画面」の場合
            // 　EVENT_ID_CM001_01LOGON2
            nextScr = Cm001Constants.EVENT_ID_CM001_01LOGON2;
            // ・「cm001_01：パスワード変更画面」の場合、DTO.パスワード変更時遷移理由を遷移先パラメタに設定する。
